import { EditorView, rectangularSelection, crosshairCursor, scrollPastEnd as cmScrollPastEnd } from '@codemirror/view';
...
import { toggleSheetEffect, frontmatterSheetField, blockRangesField } from './extensions/hybrid-preview.js';
...
export const HybridMarkdownConfig = Facet.define({
  combine(configs) {
    return {
      ...
      scrollPastEnd: configs.reduce((a, c) => c.scrollPastEnd ?? a, false),
...
/**
 * StateEffect for changing scroll past end
 * @type {StateEffectType<boolean>}
 */
const setScrollPastEndEffect = StateEffect.define();
...
const editorStateField = StateField.define({
  create(state) {
    ...
    return {
      ...
      scrollPastEnd: config.scrollPastEnd === true,
    };
  },
  update(value, tr) {
    ...
    for (const effect of tr.effects) {
      ...
      } else if (effect.is(setScrollPastEndEffect)) {
        value = { ...value, scrollPastEnd: effect.value };
      }
    }
    return value;
  },
...
export function hybridMarkdown(options = {}) {
  ...
  const extensions = [
    ...
    scrollPastEndCompartment.of(scrollPastEnd ? cmScrollPastEnd() : []),
...
export function toggleScrollPastEnd(view) {
  const currentState = view.state.field(editorStateField);
  const enabled = !currentState.scrollPastEnd;

  view.dispatch({
    effects: [
      setScrollPastEndEffect.of(enabled),
      scrollPastEndCompartment.reconfigure(enabled ? cmScrollPastEnd() : []),
    ],
  });
 ...
 * @param {EditorView} view - The editor view
 * @param {boolean} enabled - Whether scroll past end should be enabled
 */
 ...
 * @returns {boolean}
 */
export function isScrollPastEnd(view) {
  return view.state.field(editorStateField).scrollPastEnd;
}
