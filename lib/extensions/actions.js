...
export const actions = {
  ...
  link(view) {
    ...
    if (selection) {
      const { from, to } = view.state.selection.main;
      view.dispatch({
        changes: { from, to, insert: `[${selection}](${placeholder})` },
        selection: { anchor: from + selection.length + 3, head: from + selection.length + 3 + placeholder.length },
      });
    } else {
      const { from } = view.state.selection.main;
      view.dispatch({
        changes: { from, insert: `[link text](${placeholder})` },
  ...
  image(view) {
    ...
    const placeholder = 'url';
    view.dispatch({
      changes: { from, insert: `![alt text](${placeholder})` },
