...
/**
 * Callout/admonition configuration: type → { icon (SVG string), color (hex) }
 */
...
function resolveCalloutType(type) {
  ...
  const canonical = CALLOUT_ALIASES[lower] || lower;
  return CALLOUT_CONFIG[canonical] ? canonical : 'note';
}
...
/**
 * Compute callout block ranges (Obsidian-style admonitions: > [!type] Title)
 * @returns {Array<{start, end, type, title, foldable, defaultOpen}>}
 */
function computeCalloutBlockRanges(doc, codeBlockLineSet) {
  const ranges = [];
  ...
  for (let i = 1; i <= doc.lines; i++) {
    ...
    const foldModifier = match[2] || null;
    const title = match[3] || '';
    const foldable = foldModifier === '+' || foldModifier === '-';
    ...
    ranges.push({ start: i, end, type, title, foldable, defaultOpen });
    i = end;
  }
...
function computeBlockRanges(doc) {
  ...
  const calloutBlocks = computeCalloutBlockRanges(doc, codeBlockLines);
  const calloutBlockLines = buildLineSet(calloutBlocks);
...
/**
 * Widget that renders a callout/admonition block
 */
class CalloutBlockWidget extends WidgetType {
  constructor(lines, type, title, foldable, defaultOpen, blockFrom, blockTo) {
    super();
    this.lines = lines;
    this.type = type;
    this.title = title;
  ...
  toDOM(view) {
    const resolved = resolveCalloutType(this.type);
    const config = CALLOUT_CONFIG[resolved];
    const wikiLinksConfig = view.state.facet(wikiLinksFacet);
    const renderOptions = { enableWikiLinks: wikiLinksConfig.renderWikiLinks };

    const wrapper = document.createElement('div');
    wrapper.className = 'cm-callout-preview';
    wrapper.style.setProperty('--callout-color', config.color);

    // Title bar
    const titleBar = document.createElement('div');
    titleBar.className = 'cm-callout-title';
    titleBar.innerHTML = config.icon;

    const titleText = document.createElement('span');
    const displayTitle = this.title || resolved.charAt(0).toUpperCase() + resolved.slice(1);
    titleText.innerHTML = renderInline(displayTitle, renderOptions);
    ...
    // Parse content lines (skip first line which is the callout header)
    for (let i = 1; i < this.lines.length; i++) {
      const lineText = this.lines[i].replace(/^>\s?/, '');
      const p = document.createElement('p');
      p.innerHTML = lineText.trim() ? renderInline(lineText, renderOptions) : '&nbsp;';
    ...
    if (this.foldable) {
      ...
      chevron.className = 'cm-callout-fold' + (this.defaultOpen ? '' : ' collapsed');
      ...
      titleBar.addEventListener('mousedown', (e) => {
        // Only handle fold toggle, not positioning
        if (e.target.closest('.cm-callout-fold') || e.target === titleBar || titleBar.contains(e.target)) {
          e.preventDefault();
          e.stopPropagation();
    ...
    // Click to edit — only on content area (title bar may have fold handler)
    if (!this.foldable) {
      wrapper.addEventListener('mousedown', (e) => {
        const wikiLink = e.target.closest('[data-wikilink]');
        if (wikiLink && wikiLinksConfig.onWikiLinkClick) {
  ...
  eq(other) {
    ...
    if (other.foldable !== this.foldable) return false;
    if (other.defaultOpen !== this.defaultOpen) return false;
    if (other.lines.length !== this.lines.length) return false;
    for (let i = 0; i < this.lines.length; i++) {
      if (other.lines[i] !== this.lines[i]) return false;
    }
...
function buildDecorations(view, blockRanges) {
  ...
  for (const { from, to } of view.visibleRanges) {
    ...
    for (let i = startLine; i <= endLine; i++) {
      ...
      // Skip callout block lines (handled by calloutBlockDecorations plugin)
      if (calloutBlockLines.has(i)) {
        continue;
      }
...
/**
 * Build decorations for callout blocks (Obsidian-style admonitions)
 */
function buildCalloutBlockDecorations(view, blockRanges) {
  const { state } = view;
  const hasFocus = view.hasFocus;
  const decorations = [];
  const { calloutBlocks } = blockRanges;
  const focusedLines = hasFocus ? getFocusedLines(state) : new Set();

  for (const range of calloutBlocks) {
    let blockFocused = false;
    for (let j = range.start; j <= range.end; j++) {
      if (focusedLines.has(j)) {
        blockFocused = true;
    ...
    if (blockFocused) {
      for (let i = range.start; i <= range.end; i++) {
        const line = state.doc.line(i);
        decorations.push(
          Decoration.line({ class: 'cm-callout-line' }).range(line.from)
        );
      }
    } else {
      const lines = [];
      ...
      decorations.push(
        Decoration.replace({
          widget: new CalloutBlockWidget(
            lines, range.type, range.title, range.foldable, range.defaultOpen,
...
const calloutBlockDecorations = ViewPlugin.fromClass(
  class {
    constructor(view) {
      const blockRanges = view.state.field(blockRangesField);
      this.decorations = buildCalloutBlockDecorations(view, blockRanges);
    }

    update(update) {
      if (update.docChanged || update.selectionSet || update.focusChanged) {
        const blockRanges = update.state.field(blockRangesField);
        this.decorations = buildCalloutBlockDecorations(update.view, blockRanges);
      }
    }
  },
  {
